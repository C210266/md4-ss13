...
<form action="<%=request.getContextPath()%>/CustomerServlet">
    ...
    <select name="sort">
        ...
        <option value="email"
                selected="<%=request.getAttribute("sort")!=null&&request.getAttribute("sort").equals("email")%>">Email
        </option>
        <option value="address"
                selected="<%=request.getAttribute("sort")!=null&&request.getAttribute("sort").equals("address")%>">
            Address
        </option>
    </select>
    <br>
...
<table border="10" cellpadding="20" cellspacing="10" >
    ...
    <c:forEach items="${customers}" var="c" varStatus="item">
        <tr>
            ...
            <td><a onclick="return confirm('do you want to delete this item ? ')"
